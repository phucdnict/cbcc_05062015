...
// 		echo $query;die;
	...
	public function checkQuatrinhdaotao($formData){
		...
		->from($db->quoteName('quatrinhboiduongnghiepvu','qt'));
		$query->where($db->quoteName('tosc_code_dt').'='.$db->quote($id_loaitrinhdo));
		if ((int)$id_chuyennganh>0) { $query->where($db->quoteName('chuyennganh_id').'='.$db->quote($id_chuyennganh));}
		if ((int)$id_trinhdo>0) { $query->where($db->quoteName('trinhdo_code').'='.$db->quote($id_trinhdo));}
		$query->where('hosochinh_id = '.$db->quote($empid));
		$db->setQuery($query);
	 ...
	 */
	public function ds_nhucaudaotaotaidonvi($donvi_id) {
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);

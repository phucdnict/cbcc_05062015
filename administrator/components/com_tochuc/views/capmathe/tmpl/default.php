...
<div class="span4">
	...
	<legend>Gói chức vụ
				...
				<a id="btnReset" class="btn btn-small btn-info" href="index.php?option=com_tochuc&controller=capmathe&task=rebuild">
...
jQuery(document).ready(function($){
	...
	var _initEditPage = function(id){
		$.get('index.php?option=com_tochuc&controller=capmathe&task=edit&format=raw',{"id":id},function(response){
			$('#form-content').html(response);
		});
	};
	var _initNewPage = function(parent_id){
		$.get('index.php?option=com_tochuc&controller=capmathe&task=edit&format=raw',{"parent_id":parent_id},function(response){
			$('#form-content').html(response);
		});
	};
	$('#btnXoa').click(function(){
		...
		if(typeof node_id == 'undefined'){
			...
			if(confirm('Bạn có muốn xóa '+ $('#tochuc-goichucvu-tree').jstree('get_selected').text()+' ?')){
				window.location.href='index.php?option=com_tochuc&controller=capmathe&task=delete&id='+node_id;
			}
		}
	});

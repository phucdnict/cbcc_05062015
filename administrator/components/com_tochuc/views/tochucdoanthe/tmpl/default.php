...
<tr>
	...
	<th style="text-align: center;vertical-align:middle;" nowrap="nowrap" width="100">
		<?php echo JHTML::_('grid.sort', 'Sắp xếp', 'orders', $this->lists['order_Dir'], $this->lists['order'] ); ?>
	</th>
	<th style="text-align: center;vertical-align:middle;" nowrap="nowrap" width="100">
		Trạng thái
	</th>
...
for($i=0, $n=count($this->items); $i<$n; $i++){
	...
	$link_remove = JRoute::_('index.php?option=com_tochuc&controller=tochucdang&controller='.$this->data['view'].'&task=remove&dbtable='.$this->data['table'].'&id='.$row->id);
...
<tr class="<?php echo "row$k"; ?>">
	...
	<td style="text-align: center;vertical-align:middle;">
		<?php echo htmlspecialchars($row->orders); ?>
	</td>
	<td style="text-align: center;vertical-align:middle;">
		<?php 
			if($row->status == '1'){
				$link_status = JRoute::_('index.php?option=com_tochuc&controller=action&controller='.$this->data['view'].'&task=unpublish&dbtable='.$this->data['table'].'&id='.$row->id);
		?>
		...
		<?php
				...
				$link_status = JRoute::_('index.php?option=com_tochuc&controller=action&controller='.$this->data['view'].'&task=publish&dbtable='.$this->data['table'].'&id='.$row->id);
		?>
...
<input type="hidden" name="controller" value="tochucdoanthe" />

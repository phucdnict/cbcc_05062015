...
// 		echo  $query->__toString();
	...
	static public function recursive($table,$colums = array('id'=>'id','parent'=>'parentid','text'=>'name'),$where,$parentid = 0,$level = 0, &$result) {
		...
		for ($i = 0; $i < count($rows); $i++) {
			...
			$result = DaotaoHelper::recursive($table,$colums,$where,$row[$colums['id']],($level+1),&$result);
		}
    ...
    if(!is_array($outArray)) {
        ...
        foreach($inArray as $key => $tuple) {	        	        	        
	        if($tuple['parent'] == $currentParentId) {
	            ...
	            DaotaoHelper::makeParentChildRelationsForTree($inArray, $tuple['children'], $tuple['id']);
	            $outArray[] = $tuple;
	        }
	    }
	}
	...
	static public function selectBox($value,$attrs,$table,$colums,$where = null,$order = null,$optAdd = null){
		//var_dump($colums[0]);
		if (count($colums) >= 2) {		
	        $rows = DaotaoHelper::collect($table, $colums, $where, $order);
	        //var_dump($rows);exit;	
	        if (is_array($attrs)) {
	...
	static function checkFiles($file){
		...
		$convert_file = DaotaoHelper::strtolower_utf8($file['name']);

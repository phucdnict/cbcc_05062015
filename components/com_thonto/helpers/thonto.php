...
class ThontoHelper{
	// Phúc thêm
	/**
	 * Combobox không chứa text values trống
	 * @param unknown $value
	 * @param unknown $attrs
	 * @param unknown $table
	 ...
	 * @return mixed|string
	 */
	public function selectBox_notdefautl($value,$attrs,$table,$colums,$where = null,$order = null){
		if (count($colums) >= 2) {
...
// 		echo  $query->__toString();
    ...
    if(!is_array($outArray)) {
        ...
        foreach($inArray as $key => $tuple) {	        	        	        
	        if($tuple['parent'] == $currentParentId) {
	            ...
	            ThontoHelper::makeParentChildRelationsForTree($inArray, $tuple['children'], $tuple['id']);
	            $outArray[] = $tuple;
	        }
	    }
	}
	...
	static public function selectBox($value,$attrs,$table,$colums,$where = null,$order = null,$optAdd = null){
		//var_dump($colums[0]);
		if (count($colums) >= 2) {		
	        $rows = ThontoHelper::collect($table, $colums, $where, $order);
	        //var_dump($rows);exit;	
	        if (is_array($attrs)) {
	        	$controlName = $attrs['name'];
	        	unset($attrs['name']);
	...
	static public function selectCompare($val_compared,$value,$attrs,$table,$colums,$where = null,$order = null){
		if (count($colums) >= 2) {
			$rows = ThontoHelper::collect($table, $colums, $where, $order);
			if (is_array($attrs)) {
				$controlName = $attrs['name'];
				...
				if($idName != '' && $idName != null){
					$controlAttrs .=' id="'.$idName.'"';
				}else{
					$controlAttrs .=' id="'.$controlName.'"';
				}
				foreach ($attrs as $key=>$val){
					if (is_array($val)) {
						$controlAttrs .=' '.$key.'="'.implode(" ", $val).'"';
	...
	static public function getFileByIdVanban($id){
		$db = JFactory::getDbo();
		$rows = array();
		$query = $db->getQuery(true);
		$query->select('*')
				->from($db->quoteName('core_attachment'))				
				->where(array($db->quoteName('object_id').' = '.$db->q($id),$db->quoteName('type_id').' = '.$db->q('1')));
		$db->setQuery($query);
		return $db->loadAssoc();
	}
	static function checkFiles($file){
		...
		$convert_file = ThontoHelper::strtolower_utf8($file['name']);

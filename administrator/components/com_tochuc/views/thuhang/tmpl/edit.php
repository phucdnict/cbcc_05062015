...
?>
<form class="form-horizontal " action="index.php?option=com_tochuc&controller=thuhang&task=saveedit" method="post" name="frmTochucThuhang" id="frmTochucThuhang">
<fieldset>
<legend>
Thông tin chi tiết [<small><?php echo (($this->row->id == 0)?'Thêm mới':'Hiệu chỉnh');?></small>]
  ...
  <div class="control-group">
    ...
    <div class="controls">
     ...
     <?php echo AdminTochucHelper::getNameById($this->row->parent_id, 'ins_level'); ?>     
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="name">Tên</label>
    <div class="controls">
      <input type="text" id="name" placeholder="Nhập tên thứ hạng" name="name" value="<?php echo $this->row->name;?>">
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="thuhang">Thứ hạng</label>
    <div class="controls">
      <input type="text" id="thuhang" placeholder="Nhập thứ hạng" name="thuhang" value="<?php echo $this->row->thuhang;?>">
    </div>
  </div>
</fieldset>
	...
	$('#btnXoa').click(function(){
		...
		if(typeof id == 'undefined'){
			...
			if(confirm('Bạn có muốn xóa không ?')){
				window.location.href='index.php?option=com_tochuc&controller=thuhang&task=delete&id='+id;
			}
		}

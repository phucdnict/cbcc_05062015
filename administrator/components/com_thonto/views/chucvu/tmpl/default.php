...
<tr>
	...
	<th style="text-align: center;vertical-align:middle;" nowrap="nowrap">
		<?php echo JHTML::_('grid.sort', 'Tên chức vụ', 'ten', $this->lists['order_Dir'], $this->lists['order'] ); ?>
	</th>
	<th style="text-align: center;vertical-align:middle;" nowrap="nowrap" width="100">
		<?php echo JHTML::_('grid.sort', 'Loại hình tổ chức', 'loaihinhtochuc_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
	</th>
	<th style="text-align: center;vertical-align:middle;" nowrap="nowrap" width="100">
		Số tháng bổ nhiệm
	</th>
...
for($i=0, $n=count($this->items); $i<$n; $i++){
	...
	$link_remove = JRoute::_('index.php?option=com_thonto&controller=chucvu&controller='.$this->data['view'].'&task=remove&dbtable='.$this->data['table'].'&id='.$row->id);
...
<tr class="<?php echo "row$k"; ?>">
	...
	<th style="text-align: left;vertical-align:middle;" nowrap="nowrap" width="100">
		<?php 
		$thongtinChucvu = $model->getEditItem('thonto_loaihinhtochuc',$row->loaihinhtochuc_id);
		if ($row->loaihinhtochuc_id!=1)echo $thongtinChucvu[0]['ten'];?>
	</th>
	<td style="text-align: center;vertical-align:middle;">
				<?php echo htmlspecialchars($row->sothangbonhiem); ?>
	</td>
	<td style="text-align: center;vertical-align:middle;">
		<?php 
			if($row->trangthai == '1'){

...
<tr>
	...
	<th style="text-align: center;vertical-align:middle;" nowrap="nowrap">
		<?php echo JHTML::_('grid.sort', 'Tên cách thức bổ nhiệm', 'ten', $this->lists['order_Dir'], $this->lists['order'] ); ?>
	</th>
...
for($i=0, $n=count($this->items); $i<$n; $i++){
	$row = &$this->items[$i];
	$link_edit = JRoute::_('index.php?option=com_dmttcn&controller='.$this->data['view'].'&task=edit&id='. $row->id );
	$link_remove = JRoute::_('index.php?option=com_dmttcn&controller=cachthucbonhiem&controller='.$this->data['view'].'&task=remove&dbtable='.$this->data['table'].'&id='.$row->id);
...
<tr class="<?php echo "row$k"; ?>">
	...
	<td style="text-align: center;vertical-align:middle;">
		<?php 
			if($row->status == '1'){
				$link_status = JRoute::_('index.php?option=com_dmttcn&controller='.$this->data['view'].'&task=unpublish&dbtable='.$this->data['table'].'&id='.$row->id);
		?>
		...
		<?php
				...
				$link_status = JRoute::_('index.php?option=com_dmttcn&controller='.$this->data['view'].'&task=publish&dbtable='.$this->data['table'].'&id='.$row->id);
		?>

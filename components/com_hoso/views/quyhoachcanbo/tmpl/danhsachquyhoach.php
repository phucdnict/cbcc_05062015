		...
		success: function(data) {
			// P them + update 050515
				var xhtml='<h3 class="header smaller lighter blue">';
					...
					xhtml+='<th style="vertical-align: middle;" class="center">Từ năm<input style="width:30px; background-color: #f1f1f1" class="dp" id="min_date"></th>';
					xhtml+='<th style="vertical-align: middle;" class="center">Đến năm<input style="width:30px; background-color: #f1f1f1" class="dp" id="max_date"></th>';
					...
					for(i=0; i< data.length; i++){
						if (data[i].phongcongtac==null)
	 					...
	 					xhtml+='<tr  role="row">';
	 						xhtml+='<td style="vertical-align: middle;"  class="center"><input type="checkbox" class="ckb_qhcb" value="'+data[i].id_qhcb+'" style="opacity:1;position:relative"></td>';
			...
			if(showlist != '2'){
				if(showlist == 1 || showlist == 3 || showlist == 0){
					a();
	 				...
	 				$('#tabQuyhoachcanbo').html('');
	 			}
			}else {data.inst.toggle_node(data.rslt.obj);$('#tabQuyhoachcanbo').html('');}
		 });
});
</script>

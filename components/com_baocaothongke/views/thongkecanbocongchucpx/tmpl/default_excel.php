...
$activeSheet->setCellValue('D7', 'Cao cấp');
$activeSheet->setCellValue('E7', 'Trung cấp');
$activeSheet->setCellValue('F7', 'Sơ cấp');
$activeSheet->setCellValue('G7', 'Bồi dưỡng');
...
for($i=1; $i<=count($json);$i++){
	$activeSheet->setCellValue('D'.($i+7), $json[$i]->caocap_ctri);
	$activeSheet->setCellValue('E'.($i+7), $json[$i]->trungcap_ctri);
	$activeSheet->setCellValue('F'.($i+7), $json[$i]->socap_ctri);
	$activeSheet->setCellValue('G'.($i+7), $json[$i]->cunhan_ctri);
...
*/
ob_end_clean();
$excelFileName = 'Bcdaotao_congchuc';

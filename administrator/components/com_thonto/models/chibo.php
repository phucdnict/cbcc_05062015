...
class ThontoModelChibo extends JModelList{
	var $_data = null;
	var $_total = null;
	...
	public function _buildQuery($table){
		...
		$query = 'select cb.*, dv.name as tendonvi from  '.$table.'  cb LEFT JOIN ins_dept dv ON dv.id = cb.donvi_id '.$where.$orderby;
		return $query;
	}

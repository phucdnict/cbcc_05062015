...
<h3 class="row-fluid smaller lighter">
	<span class="span7">Cây tổ chức</span>
	<span class="span5">
		<span class="pull-right inline">
			<a id="btnReset" class="btn btn-small btn-success" href="index.php?option=com_tochuc&controller=tochuc&task=rebuild"><i class="icon-plus"></i> Reset</a>
...
var appAdminTochuc = {
	orderUp:function(){				
		...
		var url='index.php?option=com_tochuc&controller=tochuc&task=orderup';
	...
	orderDown:function(){
		//console.log('aa');		
		var id =  jQuery('#tochuc-goichucvu-tree').jstree('get_selected').attr('id').replace("node_","");
		var url='index.php?option=com_tochuc&controller=tochuc&task=orderdown';
		jQuery.post(url,{id:id},function(data){
			jQuery('#tochuc-goichucvu-tree').jstree("refresh");
		});		
		//console.log(id);
...
jQuery(document).ready(function($){
	...
	var _initEditPage = function(id){
		$.get('index.php?option=com_tochuc&controller=tochuc&task=edit&format=raw',{"id":id},function(response){
			$('#form-content').html(response);
		});
	};
	var _initNewPage = function(parent_id){
		$.get('index.php?option=com_tochuc&controller=tochuc&task=edit&format=raw',{"parent_id":parent_id},function(response){
			$('#form-content').html(response);
		});
	};
	$('#btnXoa').click(function(){
		...
		if(typeof node_id == 'undefined'){
			...
			if(confirm('Bạn có muốn xóa '+ $('#tochuc-goichucvu-tree').jstree('get_selected').text()+' ?')){
				window.location.href='index.php?option=com_tochuc&controller=tochuc&task=delete&id='+node_id;
			}
		}
	 ...
	 $('#tochuc-goichucvu-tree').jstree({
		...
		data.rslt.o.each(function (i) {
			$.ajax({
				async : false,
				type: 'POST',
				url: "index.php?option=com_tochuc&controller=tochuc",
				data : { 
					"task" : "movenode", 

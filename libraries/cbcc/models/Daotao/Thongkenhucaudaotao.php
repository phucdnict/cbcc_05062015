...
// 		echo $query;die;
	...
	function getThongke($donvi_id, $tungay=null, $denngay=null){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query = '  select dv.name as donvi_name,
					-- chuyên môn
					...
					inner join ins_dept dv ON dv.id = '.$donvi_id.'
					...
					-- tính tổng các node con thành phần
					  ...
					  ) AS dvbc ON (c.congtac_donvi_id = dvbc.id or c.congtac_phong_id = dvbc.id)
					where nc.trangthai = 0  ';
		$donviloaitru = Core::getUnManageDonvi(JFactory::getUser()->id, 'com_nhucaudaotao', 'treeview' ,'treethongkenhucau');
		if($donviloaitru!='')
			$query.=' and c.congtac_donvi_id NOT IN ('.$donviloaitru.') and c.congtac_phong_id NOT IN ('.$donviloaitru.')';
